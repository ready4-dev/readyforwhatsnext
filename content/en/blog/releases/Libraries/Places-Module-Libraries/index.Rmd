---
title: "Places module library releases"
linkTitle: "Places"
date: "`r Sys.Date()`"
description: "Releases of module libraries for modelling places (collectively, the Springtides model)."
weight: 3
categories: 
- Releases
tags: 
- Model
- Model - modules
- Software
- Software - libraries
output: hugodown::md_document
---

Currently available (as at  `r format(Sys.Date(),"%d-%b-%Y")`) releases of [module libraries](/docs/model/modules/finding/libraries/) for [modelling places](/docs/model/modules/using-modules/places/) are reproduced below.

````{=html}
```{r, echo=FALSE, message=F,results='asis'}
xfun::file_string('index_Body.html')
```
````

